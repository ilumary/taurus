...
Additionally, the minimum round trip time (\inlinecode{min\_rtt}) is tracked. The \inlinecode{min\_rtt} represents the sender's observation of the absolute
...
QUIC uses a congestion window to limit the number of bytes in flight (unacknowledged packets) at any given time. A sender is not allowed
